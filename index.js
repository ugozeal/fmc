  ...
  $.ajax({
    ...
    if (patientExist) {
      $("#data-display").html(`<tr>
          ...
          <td>${sickness}</td>
          <td>${status}</td>
          <td><input type="button" class="btn btn-yellow btn-general" 
          onclick="onePatient(${id})" value="Edit">&nbsp; 
          <input type="button" class="btn btn-danger btn-general" 
          onclick="deletePatient(${id})" value="delete"> </td>
      </tr>
          `);
      // alert("Exist");
...
function onePatient(i) {
  $.ajax({
    ...
    $("#replace").html(
      ...
      <div class="row">
      <div class="col-md-6">
      <input type="text" id="patient-name1" value="${resp.name}" 
      class="form-control" placeholder="Patient Name"><br>
      </div>
      
      <div class="col-md-6">
      <input type="email" id="patient-email1" value="${resp.email}" 
      class="form-control" placeholder="Patient Email"><br>
      </div>
      <div class="col-md-6">
      <input type="tel" id="patient-mobile1" value="${resp.telephone}" 
      class="form-control" placeholder="PhoneNumber"><br>
      </div>
      <div class="col-md-6">
      <input type="number" id="patient-age1" value="${resp.age}" 
      class="form-control" placeholder="Patient Age"><br>
      </div>
      <div class="col-md-6">
      <input type="text" id="patient-sickness1" value="${resp.sickness}" 
      class="form-control" placeholder="Patient Sickness"><br>
      </div>
      <div class="col-md-6">
      <input type="text" id="patient-status1" value="${resp.status}" 
      class="form-control" placeholder="Patient Status"><br>
      </div>
      </div>
      <input type="button" value="update" class="btn btn-yellow btn-general" 
      onclick="upDateInfo(${resp.id})">
      </div>
      `
    );
  });
